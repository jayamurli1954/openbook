...
* **Implementation status:** Gated — Slice 3 is eligible for authorization (implementation NOT authorized by this ADR alone)
...
**Acceptance of this ADR does not authorize implementation.** Slice 3 remains eligible for a separate explicit authorization. Slices 4–5 stay gated.
...
* Slice 3 is **eligible for implementation authorization** only. When authorized, implementation is limited to desktop asset coordination, `BookSession.addAsset()` / `removeAsset()`, image-block authoring, and tests.
* **Out of scope for Slice 3:** audio/video authoring UI; remote URL assets; image transcoding; Book Doctor UI (Slice 4); EPUB/HTML/PDF publishing or export (Slice 5); SQLite or Tauri architecture redesign; auto-save.

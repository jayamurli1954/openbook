...
@openbook/epub (EPUB 3.3 Engine)
   ...
   ├── 4. Project Navigation (nav.xhtml + legacy NCX fallback)
...
└── EPUB/
    ...
    ├── toc.ncx                          ← NCX fallback for EPUB 2 reader compatibility (optional)
    ├── styles/
...
- To ensure broad readability on older or constrained e-readers (e.g., legacy Kindle/e-ink hardware), the engine may generate a fallback `toc.ncx` file referenced in `package.opf` with `spine toc="ncx"`.
- The NCX file must remain purely a derivative fallback; `nav.xhtml` is the authoritative EPUB 3 navigation source.
...
1. **Authoritative Validator:** Official EPUBCheck 5.3.0 is the sole arbiter of EPUB 3.3 conformance (ADR-0004, ADR-0005).
2. **Zero Tolerance:** Generated EPUB publications from standard test fixtures must pass EPUBCheck with **0 errors and 0 warnings**.
3. **Integration Boundary:** Validation tests for `@openbook/epub` will invoke the existing `@openbook/validator` `ValidatorService` / `EpubCheckSubprocessAdapter` against compiled EPUB buffers.
...
6. EPUBCheck 5.3.0 zero-warning conformance gate is affirmed.

...
- **Required next product capabilities (must not be dropped; not authorized by this index):** filesystem project package (ADR-0029 Slices 1–6 complete on `main`); autosave & crash recovery (ADR-0031 Accepted; Slices 1–4 done (#98–#101), Slice 5 in progress). Gate 9 export UI / native Save As is **implemented** under ADR-0030. See `docs/IMPLEMENTATION-BACKLOG.md`.
...
ADR-0031 (Autosave & Crash Recovery) is **Accepted**. Slices 1–4 are **done** on `main` (#98–#101). Maintainer direction authorized Slice 5 only: unify explicit `saveProject` as package-first when a package root is bound (SQLite as session index). Gate 10 remains separately gated.

...
- **Required next product capabilities (must not be dropped):** filesystem project package (ADR-0029 Slices 1–6 complete); autosave & crash recovery (ADR-0031 Slices 1–5 complete); Gate 9 export UI / native Save As (ADR-0030 complete). **Gate 10 packaging/release readiness:** ADR-0032 **Accepted**; Slice 4 (Windows distributable + identity) is the current authorized unit. See `docs/IMPLEMENTATION-BACKLOG.md`.
...
ADR-0032 is **Accepted**. Slices 1–3 are **done** (PR #104, #105, #106). Slice 4 (Windows NSIS distributable + ADR-0028 identity) is authorized.

This index update does **not** authorize Slices 4–5 (Windows distributable, release-readiness verification).

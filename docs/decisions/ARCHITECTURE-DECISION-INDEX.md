...
- **Required next product capabilities (must not be dropped; not authorized by this index):** filesystem project package (ADR-0029 Slices 1–6 complete on `main`); autosave & crash recovery (ADR-0031 Proposed; Slices 1–3 done (#98–#100), Slice 4 in progress). Gate 9 export UI / native Save As is **implemented** under ADR-0030. See `docs/IMPLEMENTATION-BACKLOG.md`.
...
ADR-0031 (Autosave & Crash Recovery) is **Proposed**. Slices 1–3 are **done** on `main` (#98–#100). Maintainer direction authorized Slice 4 only: recovery discovery + coordinator open-from-package with explicit recover policy (no silent restore, no React dialog chrome). Slice 5 and Gate 10 remain separately gated.

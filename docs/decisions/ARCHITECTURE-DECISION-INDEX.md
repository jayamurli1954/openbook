...
- **Required next product capabilities (must not be dropped; not authorized by this index):** export UI / native Save As (Gate 9; architecture Accepted in ADR-0030, implementation separately gated); filesystem project package (ADR-0029 Slices 2–6); autosave & crash recovery (after atomic package Save/Open). See `docs/IMPLEMENTATION-BACKLOG.md`.
...
ADR-0030 is **Accepted**. This records the architecture only and does not authorize implementation. The Gate 9 implementation remains separately gated and must follow the accepted host/UI/filesystem boundary.

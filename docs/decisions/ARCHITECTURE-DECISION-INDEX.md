...
Production evidence population is intentionally not represented as complete. Bundled-font clearance remains evidence-dependent, and full branch-protection configuration remains not independently re-verified because of the previously recorded GitHub API 403 limitation. These limitations do not reopen ADR-0028 or authorize additional work; future operational evidence collection must follow the normal authorization sequence.
...
- Bundled-font policy — **architecture policy established by ADR-0028 Slice 3; font-by-font redistribution evidence remains pending**
...
- **Required next product capabilities (must not be dropped):** filesystem project package (ADR-0029 Slices 1–6 complete); autosave & crash recovery (ADR-0031 Slices 1–5 complete); Gate 9 export UI / native Save As (ADR-0030 complete). **Gate 10 packaging/release readiness** is the next gated unit (ADR-0032 **Proposed**; implementation not authorized). See `docs/IMPLEMENTATION-BACKLOG.md`.
...
ADR-0032 is **Proposed**. It records desktop packaging / release-readiness architecture only: a packaged-resource locator distinct from repo `.cache/`, Windows x64 as the first production packaging target, Gate 5/6 runtimes as bundled resources, fail-closed discovery with no system Java/Typst fallback, and ADR-0028 evidence consumption without production certification.

This index update does **not** accept ADR-0032 and does **not** authorize any Gate 10 implementation slice.

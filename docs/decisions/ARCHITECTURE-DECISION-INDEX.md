...
- **Required next product capabilities (must not be dropped):** filesystem project package (ADR-0029 Slices 1–6 complete); autosave & crash recovery (ADR-0031 Slices 1–5 complete); Gate 9 export UI / native Save As (ADR-0030 complete). **Gate 10 packaging/release readiness:** ADR-0032 **Accepted**; Slice 2 (Windows resource layout) is the current authorized unit. See `docs/IMPLEMENTATION-BACKLOG.md`.
...
ADR-0032 is **Accepted**. Slice 1 (packaged resource locator) is **done** (PR #104). Slice 2 (Windows resource layout + packaging-time inventory checksum verification) is authorized.

This index update does **not** authorize Slices 3–5 (desktop host wiring, Windows distributable, release-readiness verification).

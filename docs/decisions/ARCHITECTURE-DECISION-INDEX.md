...
- **Required next product capabilities (must not be dropped; not authorized by this index):** filesystem project package (ADR-0029 Slices 2–6; Slice 2 Book mapping in progress); autosave & crash recovery (after atomic package Save/Open). Gate 9 export UI / native Save As is **implemented** under ADR-0030. See `docs/IMPLEMENTATION-BACKLOG.md`.

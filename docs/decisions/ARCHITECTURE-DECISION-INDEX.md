...
| ADR-0031 | Autosave & Crash Recovery Architecture | Proposed; Slices 1–2 done (#98, #99); Slice 3 in progress | Desktop / Project persistence / Session durability | 2026-09-19 |
...
- **Required next product capabilities (must not be dropped; not authorized by this index):** filesystem project package (ADR-0029 Slices 1–6 complete on `main`); autosave & crash recovery (ADR-0031 Proposed; Slices 1–2 done (#98, #99), Slice 3 in progress). Gate 9 export UI / native Save As is **implemented** under ADR-0030. See `docs/IMPLEMENTATION-BACKLOG.md`.
...
ADR-0031 (Autosave & Crash Recovery) is **Proposed**. Slices 1–2 are **done** on `main` (#98, #99). Maintainer direction authorized Slice 3 only: coordinator dirty hooks + bound package root over `PackageAutosavePort`. Crash-recovery UX, SQLite unification, and Gate 10 remain separately gated.

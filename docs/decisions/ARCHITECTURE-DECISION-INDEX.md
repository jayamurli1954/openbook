...
| ADR-0031 | Autosave & Crash Recovery Architecture | Proposed; Slice 1 done (#98); Slice 2 in progress | Desktop / Project persistence / Session durability | 2026-09-19 |
...
- **Required next product capabilities (must not be dropped; not authorized by this index):** filesystem project package (ADR-0029 Slices 1–6 complete on `main`); autosave & crash recovery (ADR-0031 Proposed; Slice 1 done (#98), Slice 2 in progress). Gate 9 export UI / native Save As is **implemented** under ADR-0030. See `docs/IMPLEMENTATION-BACKLOG.md`.
...
ADR-0031 (Autosave & Crash Recovery) is **Proposed**. Slice 1 (`AutosaveController`) is **done** on `main` (#98). Maintainer direction authorized Slice 2 only: `PackageAutosavePort` adapting `AutosaveSavePort` to ADR-0029 `saveProjectPackage`. Coordinator wiring, recovery UX, and Gate 10 remain separately gated.

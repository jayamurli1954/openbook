...
├── Slice 3 coordinator dirty hooks        [IN PROGRESS — bound package root + mutation hooks]
...
├── Autosave & crash recovery              [REQUIRED — ADR-0031; Slice 3 in progress]
│
...
24. **ADR-0031 Autosave & Crash Recovery** — **Proposed**. Slices 1–2 **DONE** (#98, #99). Slice 3 (coordinator dirty hooks + bound package root) authorized and in progress. Remaining autosave slices and Gate 10 remain separately gated.
...
| **Autosave & crash recovery** | Losing work after the app is actually used is unacceptable. | ADR-0031 Proposed. Slices 1–2 done (#98, #99). Slice 3 coordinator wiring in progress. | Complete Slice 3, then crash-recovery UX (Slice 4). No parallel save path, no persisted Tiptap JSON. |
...
Gates 1 through 9 and ADR-0028's five implementation slices are complete on `main`. ADR-0029 Slices 1–6 are complete. ADR-0031 Slices 1–2 are done (#98, #99). Slice 3 (coordinator dirty hooks + bound package root) is the current authorized unit. Gate 10 packaging/release readiness remains separately gated.

...
├── Integrity / migration / recovery       [IN PROGRESS — Slice 5 integrity evidence; Slice 6 migration/recovery still gated]
│
...
23. **ADR-0029 Project Package architecture** — **Accepted** (#80). Slices 1–4 **DONE** (#82, #93, #94, #95). Slice 5 integrity evidence **in progress**. Slice 6 (migration/recovery) remains required and separately gated.
...
| **Filesystem project package** | Save/Open must be a versioned on-disk project, not an opaque SQLite-only session. | ADR-0029 Accepted. Slices 1–4 done (#82, #93, #94, #95). Slice 5 integrity evidence in progress. | Complete Slice 5, then Slice 6 (migration/recovery). |
...
Gates 1 through 9 and ADR-0028's five implementation slices are complete on `main`. ADR-0029 Slices 1–4 are complete. Slice 5 (integrity evidence) is in progress. Next after Slice 5: Slice 6 (migration/recovery), then autosave on that package. Gate 10 packaging/release readiness remains separately gated.

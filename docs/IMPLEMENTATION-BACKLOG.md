...
├── Integrity / migration / recovery       [IN PROGRESS — Slice 6 migration/recovery; Slice 5 integrity DONE (#96)]
│
...
23. **ADR-0029 Project Package architecture** — **Accepted** (#80). Slices 1–5 **DONE** (#82, #93, #94, #95, #96). Slice 6 migration/recovery **in progress** (closes ADR-0029 sequencing).
...
| **Filesystem project package** | Save/Open must be a versioned on-disk project, not an opaque SQLite-only session. | ADR-0029 Accepted. Slices 1–5 done (#82, #93, #94, #95, #96). Slice 6 migration/recovery in progress. | Complete Slice 6, then autosave on the package boundary (separately gated). |
...
Gates 1 through 9 and ADR-0028's five implementation slices are complete on `main`. ADR-0029 Slices 1–5 are complete. Slice 6 (migration/recovery) is in progress and closes the ADR-0029 sequencing list. Next product capability after Slice 6: autosave on the package boundary (separately gated). Gate 10 packaging/release readiness remains separately gated.

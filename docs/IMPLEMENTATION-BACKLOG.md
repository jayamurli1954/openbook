...
├── Slice 2 Windows resource layout        [IN PROGRESS — Tauri resources + packaging-time checksums]
...
├── Packaging / release readiness          [ADR-0032 Accepted; Slice 2 in progress]
│
...
25. **Gate 10 packaging / release readiness** — ADR-0032 **Accepted**. Slice 1 done (PR #104). Slice 2 (Windows resource layout) is the current authorized unit.
...
| **Packaging / release readiness** | A developer `.cache/` runtime is not a shippable desktop product. | ADR-0032 **Accepted**. Slice 1 done. Slice 2 in progress. | Complete Slice 2. Slices 3–5 remain separately gated. |
...
Gates 1 through 9, ADR-0028's five implementation slices, ADR-0029 Slices 1–6, and ADR-0031 Slices 1–5 are complete on `main`. Gate 10 ADR-0032 is **Accepted**. Slice 1 is done (PR #104). Slice 2 (Windows resource layout) is the current authorized unit.

...
├── Canonical Book persistence mapping     [IN PROGRESS — Slice 2; Book ↔ package payload mapping only]
...
23. **ADR-0029 Project Package architecture** — **Accepted** (#80). Slice 1 manifest contract **DONE** (#82). Slice 2 canonical Book persistence mapping **in progress**. Slices 3–6 (assets, atomic Save/Open, integrity/recovery) remain required and separately gated.
...
| **Filesystem project package** | Save/Open must be a versioned on-disk project, not an opaque SQLite-only session. | ADR-0029 Accepted. Slice 1 done (#82). Slice 2 Book↔package mapping in progress. | Complete Slice 2, then Slices 3–4 (assets, atomic Save/Open), then 5–6 (integrity/migration). |

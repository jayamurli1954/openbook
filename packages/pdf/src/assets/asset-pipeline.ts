...
/**
 * Processes and resolves image assets referenced by the Book for PDF publication.
 */
export async function processBookAssets(
  ...
  if (!resolver) {
    throw new AssetResolutionError(
      "MISSING_RESOLVER",
      `Book contains ${referencedAssetIds.size} referenced image block(s), but no AssetResolver was provided in PdfBuildOptions.`,
      "",
    );
  }
  ...
  for (const assetId of referencedAssetIds) {
    ...
    if (asset.kind !== "image") {
      throw new AssetValidationError(
        ...
        `Asset "${assetId}" has unsupported kind "${asset.kind}". PDF publishing supports image assets only.`,
        assetId,
      );
    }
  ...
  for (const asset of referencedAssets) {
    ...
    if (packagePathsUsed.has(normalizedPath)) {
      ...
      throw new AssetValidationError(
        "PATH_COLLISION",
        `Output path collision: assets "${asset.id}" and "${priorAssetId}" both map to "${relativePath}".`,
        asset.id,
      );
    }
    ...
    if (!asset.altText || asset.altText.trim().length === 0) {
      emitDiagnostic({
        ...
        severity: "warning",
        message: `Image asset "${asset.id}" has empty or missing altText.`,
        assetId: asset.id,
      });
    }

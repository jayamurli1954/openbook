...
export function createHash(algorithm: string): Hash {
  ...
  return {
    update(data: string) {
      const next = new TextEncoder().encode(data);
      const merged = new Uint8Array(bytes.length + next.length);
      merged.set(bytes);
      merged.set(next, bytes.length);
      bytes = merged;
      return this;
    },
    digest(encoding: "hex") {
...
function padSha256(message: Uint8Array): Uint8Array {
  ...
  const paddedLen = (withOne % 64 <= 56 ? 64 : 128) - (withOne % 64) + withOne;
  const out = new Uint8Array(paddedLen);
  out.set(message);
  out[message.length] = 0x80;
  const view = new DataView(out.buffer);
  view.setUint32(out.length - 4, bitLen, false);
  return out;
}

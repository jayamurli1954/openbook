...
test("delegates HTML export and preserves signal", async () => {
  const { coordinator, calls, result } = fakeCoordinator();
  const adapter = new ExportHostAdapter(coordinator);
  const controller = new AbortController();

  const actual = await adapter.export({ format: "html", signal: controller.signal });
...
test("delegates PDF export and preserves signal", async () => {
  ...
  const actual = await adapter.export({ format: "pdf", signal: controller.signal });
